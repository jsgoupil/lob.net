...
namespace Lob.Net.Tests
{
    public class LettersTests : BaseRequestTests
    {
        ...
        public async Task ListRequest()
        {
            var serviceCollection = GetServiceProvider(mock =>
            {
                mock.When(HttpMethod.Get, "https://api.lob.com/v1/letters")
                    .WithExactQueryString("limit=100&include%5B%5D=total_count&metadata%5Bm1%5D=v1&metadata%5Bm2%5D=v2&date_created=%7B%22gt%22%3A%222015-12-12T00%3A00%3A00.0000000%22%2C%22lt%22%3A%222017-01-01T00%3A00%3A00.0000000%22%7D&scheduled=true&send_date=%7B%22gt%22%3A%222016-12-12T00%3A00%3A00.0000000%22%2C%22lt%22%3A%222018-01-01T00%3A00%3A00.0000000%22%7D&mail_type=usps_first_class&sort_by%5Bdate_created%5D=asc&color=true")
        ...
        public async Task ListObjectsRequest()
        {
            ...
            var letters = serviceProvider.GetService<ILobLetters>();
            var enumerable = letters.ListObjectsAsync(new LetterFilter
            {
                Color = true,
                CreatedAfter = new DateTime(2015, 12, 12),
                CreatedBefore = new DateTime(2017, 1, 1),
            ...
            var list = new List<LetterResponse>();
