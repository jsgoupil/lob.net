...
namespace Lob.Net.Tests
{
    public class PostcardsTests : BaseRequestTests
    {
        ...
        public async Task ListRequest()
        {
            var serviceCollection = GetServiceProvider(mock =>
            {
                mock.When(HttpMethod.Get, "https://api.lob.com/v1/postcards")
                    .WithExactQueryString("limit=100&include%5B%5D=total_count&metadata%5Bm1%5D=v1&metadata%5Bm2%5D=v2&date_created=%7B%22gt%22%3A%222015-12-12T00%3A00%3A00.0000000%22%2C%22lt%22%3A%222017-01-01T00%3A00%3A00.0000000%22%7D&scheduled=true&send_date=%7B%22gt%22%3A%222016-12-12T00%3A00%3A00.0000000%22%2C%22lt%22%3A%222018-01-01T00%3A00%3A00.0000000%22%7D&mail_type=usps_first_class&sort_by%5Bdate_created%5D=asc&size=4x6")
        ...
        public async Task ListObjectsRequest()
        {
            ...
            var postcards = serviceProvider.GetService<ILobPostcards>();
            var enumerable = postcards.ListObjectsAsync(new PostcardFilter
            {
                CreatedAfter = new DateTime(2015, 12, 12),
                CreatedBefore = new DateTime(2017, 1, 1),
                IncludeTotalCount = true,
            ...
            var list = new List<PostcardResponse>();
